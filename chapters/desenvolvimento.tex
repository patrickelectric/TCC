...
O \textit{toolkit} foi utilizado para extração das opções de configuração, podendo dar destaque ao arquivo \textit{boards.txt} que contem informações pertinentes das placas da Arduino.
...
\begin{itemize}
\item \textit{upload.tool} - Ferramenta para envio do código de máquina (linha 3).

\item \textit{upload.protocol} - Protocolo de comunicação com o \textit{bootloader} (linha 4).
%http://www.atmel.com/images/Atmel-8271-8-bit-AVR-Microcontroller-ATmega48A-48PA-88A-88PA-168A-168PA-328-328P_datasheet_Complete.pdf

%http://www.avrfreaks.net/forum/self-programming-and-lock-bits

\item \textit{bootloader.tool} - Ferramenta responsável pela comunicação com o \textit{bootloader} (linha 6).

\item \textit{bootloader.unlock\_bits} - Bits para desbloquear o \textit{bootloader} na realização de escrita (linha 7).

\item \textit{bootloader.lock\_bits} - Bits para bloquear o \textit{bootloader} na realização de escrita (linha 8).
...
\item \textit{build.f\_cpu} - Clock do processador programado (linha 10), configurado via \textit{bootloader}.

\item \textit{build.board} - Nome da placa (linha 11).

\item \textit{build.core} - Tipo de hardware (linha 12).

\item \textit{build.variant} - Topologia do hardware (linha 13).

\item \textit{build.mcu} - Processador utilizado \cite{ATmega48A} (linha 28/43).

\item \textit{upload.maximum\_size} - Espaço livre na \textit{flash}, descontando \textit{bootloader} (linha 19/34).

\item \textit{upload.maximum\_data\_size} - Espaço livre na \textit{RAM} (linha 20/35).

\item \textit{upload.speed} - Velocidade da comunicação com o \textit{bootloader} (linha 21/36).

%http://archive.fabacademy.org/archives/2016/doc/fuses.html
\item \textit{upload.low\_fuses} - Bits menos significativos de configuração do comportamento do hardware (linha 23/38).

\item \textit{upload.high\_fuses} - Bits mais significativos de configuração do comportamento do hardware (linha 24/39).

\item \textit{upload.extended\_fuses} - Bits estendidos de configuração do comportamento do hardware (linha 25/40).

\item \textit{upload.bootloader\_file} - Nome do arquivo que contem o código de máquina do sistema de \textit{bootloader} (linha 26/41).
...
O plugin foi integrado no KDevelop (\figref{fig:kdevelop}) via a utilização de ferramentas básicas de desenvolvimento para o mesmo (\textit{kdevplatform}). Nesta seção, será demonstrando as interfaces desenvolvidas e sua integração com o KDevelop.
...
\subitem \textbf{\textit{Download} (Down)}: É realizado a comunicação com o servidor oficial, utilizado um nome randômico durante o download por segurança para o instalador (\figref{fig:kdevelopinstaller21}).
...
\begin{figure}[htb!]
  ...
  \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2.8cm,semithick]
    ...
    \path (A) edge              node {Instalação Existente}      (E)
	      (A) edge  [bend left] node {Instalação Automatica}     (B)
          (B) edge              node {Iniciado}                  (C)
          (C) edge              node {Finalizado}                (D)
          (D) edge              node {Instalado}                 (E);
  \end{tikzpicture}
...
Após realizar as configurações, basta aplicar e em seguida executar o lançamento com o botão Execute. Tendo isso realizado, será aberto um console onde todos os dados de gravação serão mostrados (\figref{fig:runavrdude} e \figref{fig:runopenocd}).
