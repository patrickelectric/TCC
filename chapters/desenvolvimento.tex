...
O conjunto de ferramentas do Arduino foram utilizados para extração das opções de configuração, podendo dar destaque ao arquivo boards.txt 

\lstinputlisting[caption={Parte de boards.txt},label=sin2]{boards.txt}
...
\subitem Tanto o \textit{unlock\_bits} quanto o \textit{lock\_bits} controla a programação externa, utilizando um sistema especializado, como ISP (\textit{In-Circuit Serial Programmer}), JTAG, entre outros \cite{fuseSettings}. 
...
%openocd -f board/ek-lm4f232.cfg -c init -c targets -c "halt" -c "flash write_image erase ${kk}" -c "verify_image ${kk}" -c "reset run" -c shutdown
